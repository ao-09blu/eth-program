...
G = [[] for _ in range(N)] #G[i]:頂点iを視点とする変情報を格納
for i in range(M):
    u, v, w = map(int, input().split())
    G[u].append(edge(v, w))

dist = [INF for _ in range(N)] #dist[i]:頂点0から頂点iへの暫定的な経路長
dist[0] = 0
done = [False for _ in range(N)] # done[i]:頂点iの最短距離が確定しているか

hq = [] #仮の最短距離、頂点番号を管理するヒープ
heapq.heapify(hq)
...
while len(hq) > 0:
    #ヒープの先頭要素を取り出す(vは頂点番号、dは0→vの仮の最短経路)
    [d, v] = heapq.heappop(hq)
    #頂点vの最短距離がすでに確定しているなら、何もしない
    if done[v]: continue

    #頂点vを視点とする変eについて更新を行う
    for e in G[v]:
...
def make_topology(link_DB, router_DB ,myRouterID):
    router_flag = [0]*
    for i in range(len(router_DB)):
        link_DB[]
...
P_key="74 143 100 108 115 122 196 137 67 251 20 77 234 111 86 30 32 89 0 242 147 137 247 200 167 142 144 61 123 104 92 122"
...
tx_hash =w3.eth.sendRawTransaction(signed_tx.rawTransaction)
tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
hop_count = [-1] * 10 #hop数を管理するカウント
#-1の場合はまだ登録されていない、0なら自分のアカウント
hop_count[my_router_id] = 0#自分のところを0に設定
